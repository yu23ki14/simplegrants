    ...
    const fetchInitialData = async () => {
      ...
      for (const grant of grants) {
        ...
        for (let amount = 0; amount <= 2000; amount += 100) {
          const matchingAmount = await fetchMatchingAmountEstimate(
            amount,
            grant.id
          )
    ...
    const datasets = grants.map((grant, index) => {
      ...
      return {
        ...
        pointBackgroundColor: dataPoints.map((dp) =>
          dp.x === grant.amount ? "red" : "rgba(0, 0, 255, 0.5)"
        ), // 寄付額と一致する点を赤色にする
      }
    })
  ...
  const totalMatchingAmount = React.useMemo(() => {
    return grants.reduce((acc, grant) => {
      const matchingAmount =
        chartData[grant.id]?.find((data) => data.x === grant.amount)?.y || 0
                          ...
                          <div className="flex flex-row items-center">
                            ...
                            <div className="flex flex-col">
                              ...
                              <p className="text-lg font-bold">
                                {chartData[grant.id]
                                  ? Math.round(
                                      chartData[grant.id].find(
                                        (data) => data.x === grant.amount
                                      )?.y || 0
                                    ).toLocaleString()
                                  : "計算中..."}{" "}
                                円
